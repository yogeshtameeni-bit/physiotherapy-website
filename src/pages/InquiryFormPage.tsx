...
type InquiryFormData = Omit<Patient, "id">;

const emptyInquiry: InquiryFormData = {
  branchId: 0,
  fullName: "",
  gender: "",
  age: 0,
...
export default function InquiryFormPage() {
  ...
  const [form, setForm] = useState<InquiryFormData>(emptyInquiry);
    ...
    <Box ref={pageRef} sx={{ maxWidth: 900 }}>
      <Paper
        ...
        <Typography sx={{ opacity: 0.84 }}>
          Make phone number unique to avoid duplicates. You can edit this information later.
        </Typography>
      </Paper>
      ...
      <Paper component="form" ref={formRef} onSubmit={handleSubmit} sx={{ p: { xs: 2, sm: 3 }, borderRadius: 2 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 2 }}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={form.isInquiryConvertedToPatient}
                onChange={(event) => updateField("isInquiryConvertedToPatient", event.target.checked)}
                sx={{ color: "primary.main", '&.Mui-checked': { color: "primary.main" } }} />
            }
            label="Is Inquiry Converted to Patient"
            sx={{
              color: "primary.main",
              fontWeight: 700,
